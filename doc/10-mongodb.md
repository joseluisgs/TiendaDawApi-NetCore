...
MongoDB es una base de datos NoSQL orientada a documentos que almacena datos en formato JSON/BSON. En el proyecto TiendaApi, usamos MongoDB como base secundaria para datos que no requieren la estructura relacional de PostgreSQL, como logs de auditoría, eventos, y datos históricos. Hay dos formas de usar MongoDB en .NET: con **EF Core para MongoDB** (recomendado para consistencia con PostgreSQL) y con el **MongoDB Driver nativo** (para casos específicos).
